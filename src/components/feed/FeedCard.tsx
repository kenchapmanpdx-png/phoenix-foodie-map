...
export default function FeedCard({ content, compact = false }: FeedCardProps) {
  ...
  /* ── Compact grid card (same layout as VideoCard compact) ── */
  if (compact) {
    return (
      <Link href={`/content/${content.id}`}>
        ...
        >
          ...
          {/* Save */}
          <button onClick={handleSaveClick} className="absolute top-2 right-2 z-10 p-1.5 glass-light rounded-full btn-press" aria-label="Save">
            <svg className={`w-3.5 h-3.5 transition-all duration-300 ${isSaved ? 'fill-[var(--color-accent-primary)] text-[var(--color-accent-primary)]' : 'text-white'}`} viewBox="0 0 24 24" fill={isSaved ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2.5}>
          ...
          {(content.restaurant.creator_count ?? 0) >= 3 && (
            <div className="absolute top-2 left-2 z-10 badge-creator-pick inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-2.5 z-[5]">
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="text-[10px] text-white/70 font-medium truncate">{content.creator.display_name}</span>
            </div>
            <h3 className="text-xs font-bold text-white line-clamp-2 leading-tight">{content.restaurant.name}</h3>
      ...
      >
        ...
        <div className="absolute top-3 left-3 z-10 flex items-center gap-2">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-orange-600 flex-shrink-0 overflow-hidden ring-2 ring-white/20 shadow-lg">
            {content.creator.avatar_url && (<img src={content.creator.avatar_url} alt="" className="w-full h-full object-cover" />)}
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-semibold text-white truncate max-w-[130px] drop-shadow-md">{content.creator.display_name}</span>
        ...
        {/* Badges */}
        {(content.restaurant.creator_count ?? 0) >= 3 && (
          <div className="absolute top-12 left-3 z-10 badge-creator-pick badge-animated inline-flex items-center gap-1 px-2 py-0.5 rounded-full">
